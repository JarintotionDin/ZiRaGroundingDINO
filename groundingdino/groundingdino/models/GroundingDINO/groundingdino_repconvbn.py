...
def conv_bn(in_channels: int, out_channels: int, kernel_size: _size_2_t, stride: _size_2_t = 1, padding = 0,
            dilation: _size_2_t = 1, groups: int = 1, bias: bool = True, padding_mode: str = 'zeros', device=None, dtype=None, zero_value=zero_value):
    ...
    nn.init.constant_(result.conv.weight, val=zero_value)
    if result.conv.bias is not None:
        nn.init.constant_(result.conv.bias, val=zero_value)
    nn.init.constant_(result.bn.weight, val=zero_value)
    nn.init.constant_(result.bn.bias, val=zero_value)
    result.bn.running_mean = torch.ones_like(result.bn.running_mean) * zero_value
    result.bn.running_var = torch.ones_like(result.bn.running_var) * zero_value
    return result


class ZeroConv2dbn(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: _size_2_t, stride: _size_2_t = 1, padding = 0,
        ...
        super().__init__()
        self.branch = conv_bn(in_channels, out_channels, kernel_size, stride, padding,
                              dilation, groups, bias, padding_mode, device, dtype, zero_value)
        self.freeze_conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding,
                              dilation, groups, bias, padding_mode, device, dtype)
        nn.init.constant_(self.freeze_conv.weight, val=0.0)
        if self.freeze_conv.bias is not None:
    ...
    def forward(self, input: Tensor) -> Tensor:
        if self.training:
            branch_output = self.branch(input)
    ...
    def __rep__(self):
        rep_weight, rep_bias = self._fuse_bn_tensor(self.branch)
        self.freeze_conv.weight.data = rep_weight  + self.freeze_conv.weight.data
        self.freeze_conv.bias.data = rep_bias * self.freeze_conv.bias.data
        
        nn.init.constant_(self.branch.conv.weight, val=zero_value)
        if self.branch.conv.bias is not None:
            nn.init.constant_(self.branch.conv.bias, val=zero_value)
        nn.init.constant_(self.branch.bn.weight, val=zero_value)
        nn.init.constant_(self.branch.bn.bias, val=zero_value)
        self.branch.bn.running_mean = torch.ones_like(self.branch.bn.running_mean) * self.zero_value
        self.branch.bn.running_var = torch.ones_like(self.branch.bn.running_var) * self.zero_value
    ...
    def __init__(
        ...
        if use_project_adapter:
            if num_feature_levels > 1:
                ...
                for _ in range(num_backbone_outs):
                    in_channels = backbone.num_channels[_]
                    input_proj_list.append(ZeroConv2dbn(in_channels, hidden_dim, kernel_size=1))
                for _ in range(num_feature_levels - num_backbone_outs):
                    input_proj_list.append(ZeroConv2dbn(in_channels, hidden_dim, kernel_size=3, stride=2, padding=1))
                    in_channels = hidden_dim
                self.input_proj_conv_adapter = nn.ModuleList(input_proj_list)
            else:
                assert two_stage_type == "no", "two_stage_type should be no if num_feature_levels=1 !!!"
                self.input_proj_conv_adapter = nn.ModuleList([ZeroConv2dbn(backbone.num_channels[-1], hidden_dim, kernel_size=1)])
    ...
    def forward(self, batched_inputs, **kw):
        ...
        if self.training:
            ...
            if self.use_project_adapter and self.use_zero_inter_loss_for_conv:
                ...
                loss_dict["loss_conv_adapter"] = loss_conv_adapter * 0.05
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name="repconvbngroundingdino")
def build_repconvbn_groundingdino(args):
