...
def _get_coco_fewshot_instances_meta():
    ...
    total_categories = [k for k in COCO_CATEGORIES if k["isthing"] == 1]
    ret["target_dataset_cls_id"] =[k["id"] for k in total_categories]
    ret["target_cls"] = range(len(total_categories))
    return ret
