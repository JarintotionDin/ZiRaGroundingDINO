...
__all__ = ["DetrDatasetMapper", "SelfSupervisionDetrDatasetMapper"]
...
class DetrDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        captions = ".".join(categories_names) + "."
        ...
        if "annotations" in dataset_dict:
            ...
            # categories_names = [self.categories_names[int(obj["category_id"])] for obj in annos]

            dataset_dict["instances"] = utils.filter_empty_instances(instances)
        return dataset_dict


class SelfSupervisionDetrDatasetMapper:
    """
    A callable which takes a dataset dict in Detectron2 Dataset format,
    and map it into the format used by DETR.
    ...
    def selective_search(self, img, h, w, res_size=128):
        ...
        ss.switchToSelectiveSearchFast(50 + int(np.random.rand(1) * 100),
                                       50 + int(np.random.rand(1) * 100))
    ...
    def __call__(self, dataset_dict):
        ...
        if self.is_train:
            patch_boxes = self.selective_search(img=image, h=image.shape[0], w=image.shape[1], res_size=128)
            dataset_dict["patch_boxes"] = patch_boxes
