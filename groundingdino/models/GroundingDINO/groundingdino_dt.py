        ...
        if use_project_adapter:
            if num_feature_levels > 1:
                ...
                for _ in range(num_backbone_outs):
                    ...
                    input_proj_list.append(
                        nn.Sequential(
                            ZeroConv2d(in_channels, hidden_dim, kernel_size=1),
                            ZeroGroupNorm(32, hidden_dim),
                        )
                    )
                for _ in range(num_feature_levels - num_backbone_outs):
                    input_proj_list.append(
                        nn.Sequential(
                            ZeroConv2d(in_channels, hidden_dim, kernel_size=3, stride=2, padding=1),
                            ZeroGroupNorm(32, hidden_dim),
                        )
                    )
                    in_channels = hidden_dim
                ...
                assert two_stage_type == "no", "two_stage_type should be no if num_feature_levels=1 !!!"
                self.input_proj_conv_adapter = nn.ModuleList(
                    [
                        nn.Sequential(
                            ZeroConv2d(backbone.num_channels[-1], hidden_dim, kernel_size=1),
                            ZeroGroupNorm(32, hidden_dim),
                        )
                    ]
                )
    ...
    def forward(self, batched_inputs, **kw):
        ...
        for l, feat in enumerate(features):
            ...
            if not self.use_project_adapter:
                ...
                conv_adapter_output = self.input_proj_conv_adapter[l](src)
                srcs.append(self.input_proj[l](src) + conv_adapter_output)
                if loss_conv_adapter is None:
                    loss_conv_adapter = self.adapter_loss(conv_adapter_output, torch.zeros_like(conv_adapter_output))
                else:
                    loss_conv_adapter = loss_conv_adapter + self.adapter_loss(conv_adapter_output, torch.zeros_like(conv_adapter_output))
            masks.append(mask)
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    if not self.use_project_adapter:
                        ...
                        conv_adapter_output = self.input_proj_conv_adapter[l](features[-1].tensors)
                    ...
                    if not self.use_project_adapter:
                        ...
                        conv_adapter_output = self.input_proj_conv_adapter[l](srcs[-1])
...
def build_dt_groundingdino(args):
