...
def do_train(args, cfg, replay=False):
    ...
    logger.info("Model:\n{}".format(model))
    print("loading model from: ",  args.model_checkpoint_path)
...
def main(args):
    ...
    if not args.eval_only: 
        ...
        shutil.copy(args.model_checkpoint_path, final_model_path)
        args.model_checkpoint_path = final_model_path
    ...
    for k, v in avg_res.items():
        if k != coco_config_file:
            sum += v
    logger.info("average AP: {}".format(sum / (len(avg_res) - 1)))
